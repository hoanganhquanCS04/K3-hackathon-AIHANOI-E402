...
| Kích thước vector | 768 chiều, yêu cầu trực tiếp qua tham số `dimensions` |
...
### 6.1. Lý do chọn vector 768 chiều

`text-embedding-3-small` trả vector 1536 chiều theo mặc định, nhưng OpenAI cho phép yêu cầu trực tiếp kích thước output nhỏ hơn thông qua tham số `dimensions`. Bản hackathon thống nhất sử dụng:
...
Khi đó mỗi embedding trả về phải có đúng 768 phần tử và Qdrant collection phải được tạo với `size=768`.

Lý do chọn 768 chiều:

- Giảm khoảng một nửa dung lượng vector so với output mặc định 1536 chiều.
- Giảm RAM, disk và băng thông khi upload/query Qdrant.
- Vẫn sử dụng trực tiếp khả năng rút gọn dimension của OpenAI, không tự cắt vector thủ công.
- 768 chiều là mức cân bằng hợp lý để nhóm bắt đầu đánh giá retrieval tiếng Việt trong phạm vi hackathon.

Đây là một quyết định chất lượng–tài nguyên cần được khóa trong manifest và kiểm chứng bằng retrieval evaluation. Không được tạo embedding 1536 chiều rồi tự lấy 768 phần tử đầu trong pipeline. Mọi request ingest và query đều phải truyền `dimensions=768`.

Nếu sau này đổi sang dimension khác, phải:
...
- Mỗi vector có đúng 768 phần tử.
...
```json
{
  "collection_name": "vlearn_transcripts_openai_small_768_v1",
  "schema_version": "1.0",
...
```text
OpenAI embedding trả vector 768 chiều.
...
- [ ] Collection dùng vector size 768 và Cosine.
